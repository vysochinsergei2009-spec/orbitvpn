...
async def attempt_auto_renewal(bot: Bot, user: User, redis) -> bool:
    try:
...
async def check_auto_renewals(bot: Bot) -> None:
    try:
        ...
        async with get_session() as session:
            ...
            for user in users:
                ...
                success = await attempt_auto_renewal(bot, user, redis)
                if success:
                    renewal_count += 1
