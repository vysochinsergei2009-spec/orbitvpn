...
def get_random_message(lang: str, days: int | str, user_balance: float = 0) -> str:
...
async def send_notification(bot: Bot, tg_id: int, lang: str, days: int | str, user_balance: float = 0) -> bool:
    try:
        message = get_random_message(lang, days, user_balance)
        if not message:
            return False
...
async def check_and_notify_user(bot: Bot, user: User, redis) -> None:
    ...
    success = await send_notification(bot, user.tg_id, user.lang, days, float(user.balance))
...
async def check_expiring_subscriptions(bot: Bot) -> None:
    try:
        ...
        async with get_session() as session:
            ...
            for user in users:
                await check_and_notify_user(bot, user, redis)
